...
EspMQTTClient client(
  ...
  "STATION_FIVE",      // Client name that uniquely identify your device
  1883
);
...
void onConnectionEstablished() {
  ...
  client.subscribe("actuator/6", [](const String & payload) {
...
void loop() {
  ...
  handleSensor(D2, count_one, noSightCount_one, lastEdge_one, "sensor/10");
  handleSensor(D5, count_two, noSightCount_two, lastEdge_two, "sensor/12");
  handleSensor(D6, count_three, noSightCount_three, lastEdge_three, "sensor/11");
  client.loop();
  delay(DELAY_MS);
}
