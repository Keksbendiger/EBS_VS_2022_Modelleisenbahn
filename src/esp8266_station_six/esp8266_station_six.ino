...
EspMQTTClient client(
  ...
  "STATION_SIX",      // Client name that uniquely identify your device
  1883
);
...
void onConnectionEstablished() {
  ...
  client.subscribe("actuator/5", [](const String & payload) {
...
void loop() {
  ...
  handleSensor(D2, count_one, noSightCount_one, lastEdge_one, "sensor/9");
  handleSensor(D5, count_two, noSightCount_two, lastEdge_two, "sensor/7");
  handleSensor(D6, count_three, noSightCount_three, lastEdge_three, "sensor/8");
  client.loop();
  delay(DELAY_MS);
}
